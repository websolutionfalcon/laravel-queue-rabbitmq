...
class DelayStrategyFactory
{
     ...
     *
     * Note: Plugin detection is deferred until the first delayed message is published
     * to avoid unnecessary checks during queue initialization.
     *
     * @param  string  $strategy  The strategy name ('dlx' or 'plugin')
     * @param  RabbitMQQueue  $queue  The RabbitMQ queue instance
     * @return DelayStrategyInterface The delay strategy instance
    ...
    {
        // For DLX strategy, just create it
        if (strtolower($strategy) === 'dlx') {
            return new DLXDelayStrategy($queue);
        }

        // For plugin strategy, return a wrapper that will check on first use
        return new PluginDelayStrategyWithFallback($queue);
    }
}
